...
CLICK_DELAY = 0.05
EXIT_DWELL_FRAMES = 40  # Yanlışlıkla çıkmamak için süreyi biraz artırdık (yaklaşık 1.5 sn)
...
# ---------------------------------------------------------
# YARDIMCI SINIF: ÇIKIŞ BUTONU (Görsel Geri Bildirimli)
# ---------------------------------------------------------
class ExitOverlay(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.progress = 0  # 0 ile 100 arası
    ...
    def paintEvent(self, event):
        ...
        # Arka plan (Yarı saydam kırmızı)
        p.setBrush(QColor(200, 0, 0, 200))
        p.setPen(Qt.NoPen)
        rect = self.rect()
        ...
        # İlerleme Çubuğu (Daire şeklinde dolum)
        if self.progress > 0:
            p.setBrush(QColor(0, 255, 0, 150))  # Yeşil dolum
            # Ortaya bir pasta dilimi çizerek dolumu göster
            center_x = rect.width() / 2
            center_y = rect.height() / 2
            radius = min(center_x, center_y) - 10

            # Açıyı hesapla (progress 100 ise 360 derece)
            span_angle = int(-360 * 16 * (self.progress / 100))
            p.drawPie(int(center_x - radius), int(center_y - radius),
...
class VideoThread(QThread):
    ...
    exit_progress_signal = pyqtSignal(int)  # Yeni sinyal: İlerleme durumu
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                if self.mouse_mode_active:
                    # --- GÜNCELLENMİŞ ÇIKIŞ MANTIĞI ---
                    cursor_x, cursor_y = pyautogui.position()

                    # Toleransı artırdık: 200x200 piksel içine bakmak yeterli
                    if cursor_x < 200 and cursor_y < 200:
                        self.exit_dwell_counter += 1

                        # İlerlemeyi hesapla ve GUI'ye gönder
                        progress = int((self.exit_dwell_counter / EXIT_DWELL_FRAMES) * 100)
                        self.exit_progress_signal.emit(progress)

                        if self.exit_dwell_counter > EXIT_DWELL_FRAMES:
                            self.exit_mouse_mode_signal.emit()
                            self.exit_dwell_counter = 0
                            self.exit_progress_signal.emit(0)  # Sıfırla
                    else:
                        # Eğer kullanıcı dışarı bakarsa sayacı yavaşça değil, direkt sıfırla
                        if self.exit_dwell_counter > 0:
                            self.exit_dwell_counter = 0
                            self.exit_progress_signal.emit(0)
                    ...
                    if is_left_closed and is_right_closed:
                        if self.scroll_ref_y is None: self.scroll_ref_y = nose_y
                        diff = nose_y - self.scroll_ref_y
                        if abs(diff) > 0.02:
                            pyautogui.scroll(int(diff * 50 * 3))
                    else:
                        self.scroll_ref_y = None
...
class RadialKeyboard(QWidget):
    ...
    def draw_keys(self, p, c, r):
        k = self.layouts[self.current_mode]
        step = 360 / len(k)
        for i, ky in enumerate(k): self.draw_pie_slice(p, c, r, -(i * step), -step, ky, ky == self.hovered_key)
...
class MainApp(QMainWindow):
    def __init__(self):
        ...
        # YENİ: QLabel yerine özel ExitOverlay widget'ı
        ...
        # Sinyal bağlantısı:
        self.thread.exit_progress_signal.connect(self.update_exit_progress)
        self.thread.start()
    ...
    def update_exit_progress(self, val):
        if self.exit_btn.isVisible():
            self.exit_btn.set_progress(val)
    ...
    def on_key(self, k):
        if k == "MOUSE": self.start_mouse_mode(); return
        if k.startswith("["): self.log(k); return
        try:
            if k == "ENTER":
                pyautogui.press('enter'); self.log("ENTER")
            elif k == "BACK":
                pyautogui.press('backspace'); self.log("SİL")
            elif k == "SPACE":
                pyautogui.press('space'); self.log("BOŞLUK")
            elif k == "MODE":
                pass
            else:
                pyautogui.write(k); self.log(f"Yazıldı: {k}")
        except:
            pass
    ...
    def start_mouse_mode(self):
        ...
        # Mouse moduna girerken progress'i sıfırla
        self.exit_btn.set_progress(0)
        print("MOUSE MODU: Maske aktif, arka plan açık.")
